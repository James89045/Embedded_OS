...
static ssize_t etx_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    ...
    //reading GPIO value
    gpio_state = gpio_get_value(GPIO_2);
    //write to user
    len = 1;
...
static int __init etx_driver_init(void)
{
    ...
    if(gpio_is_valid(GPIO_2) == false || gpio_is_valid(GPIO_3) == false || gpio_is_valid(GPIO_4) == false || gpio_is_valid(GPIO_17) == false || gpio_is_valid(GPIO_27) == false || gpio_is_valid(GPIO_22) == false || gpio_is_valid(GPIO_10) == false) {
        pr_err("GPIO %d is not valid\n", GPIO_2);
        goto r_device;
    }
    //Requesting the GPIO
    if(gpio_request(GPIO_2,"GPIO_2") < 0 || gpio_request(GPIO_4,"GPIO_4") < 0 || gpio_request(GPIO_3,"GPIO_3") < 0 || gpio_request(GPIO_17,"GPIO_17") || gpio_request(GPIO_27,"GPIO_27") < 0 || gpio_request(GPIO_22,"GPIO_22") < 0 || gpio_request(GPIO_10,"GPIO_10") < 0) {
